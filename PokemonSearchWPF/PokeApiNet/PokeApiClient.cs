...
public class PokeApiClient : IDisposable
{
    ...
    private readonly Uri _baseUri = new("http://localhost/api/v2/");
    ...
    public async Task<T> GetResourceAsync<T>(int id) where T : ResourceBase =>
        await GetResourceAsync<T>(id, CancellationToken.None);
    ...
    public async Task<T> GetResourceAsync<T>(string name)
        ...
        await GetResourceAsync<T>(name, CancellationToken.None);
    ...
    /// </summary>
    /// <typeparam name="T">The type of resource</typeparam>
    /// <param name="partialName">Part of the name of the resource</param>
    /// <returns>The object of the resource</returns>
    public async Task<NamedApiResourceList<T>> SearchNamedResourceAsync<T>(string partialName)
        where T : NamedApiResource =>
        await SearchNamedResourceAsync<T>(partialName, CancellationToken.None);
    ...
    /// <param name="cancellationToken">Cancellation token for the request; not utilized if data has been cached</param>
    /// <returns>The object of the resource</returns>
    public async Task<NamedApiResourceList<T>> SearchNamedResourceAsync<T>(string partialName, CancellationToken cancellationToken)
        where T : NamedApiResource
    {
        string apiEndpoint = GetApiEndpointString<T>();
        ...
        string url = QueryHelpers.AddQueryString(apiEndpoint, queryParameters);

        return await GetAsync<NamedApiResourceList<T>>(url, cancellationToken);
    }
    ...
    public async Task<List<T>> GetResourceAsync<T>(IEnumerable<UrlNavigation<T>> collection)
        where T : ResourceBase =>
        await GetResourceAsync<T>(collection, CancellationToken.None);
    ...
    public async Task<List<T>> GetResourceAsync<T>(IEnumerable<UrlNavigation<T>> collection, CancellationToken cancellationToken)
        where T : ResourceBase =>
        (await Task.WhenAll(collection.Select(m => GetResourceAsync(m, cancellationToken)))).ToList();
    ...
    public async Task<T> GetResourceAsync<T>(UrlNavigation<T> urlResource)
        where T : ResourceBase => await GetResourceByUrlAsync<T>(urlResource.Url, CancellationToken.None);
    ...
    public async Task<T> GetResourceAsync<T>(UrlNavigation<T> urlResource, CancellationToken cancellationToken)
        where T : ResourceBase => await GetResourceByUrlAsync<T>(urlResource.Url, cancellationToken);
    ...
    public void ClearResourceCache<T>()
        where T : ResourceBase => _resourceCache.Clear<T>();

    /// <summary>
    /// Clears the cached data for all resource types
    /// </summary>
    public void ClearResourceCache() => _resourceCache.ClearAll();

    /// <summary>
    /// Clears the cached data for all resource lists
    /// </summary>
    public void ClearResourceListCache() => _resourceListCache.ClearAll();
    ...
    public void ClearResourceListCache<T>()
        where T : ResourceBase => _resourceListCache.Clear<T>();
    ...
    public Task<NamedApiResourceList<T>> GetNamedResourcePageAsync<T>(CancellationToken cancellationToken = default)
        where T : NamedApiResource =>
        InternalGetNamedResourcePageAsync<T>(AddPaginationParamsToUrl(GetApiEndpointString<T>()), cancellationToken);
    ...
    public Task<NamedApiResourceList<T>> GetNamedResourcePageAsync<T>(int limit, int offset, CancellationToken cancellationToken = default)
        where T : NamedApiResource =>
        InternalGetNamedResourcePageAsync<T>(AddPaginationParamsToUrl(GetApiEndpointString<T>(), limit, offset), cancellationToken);
    ...
    public async IAsyncEnumerable<NamedApiResource<T>> GetAllNamedResourcesAsync<T>([EnumeratorCancellation] CancellationToken cancellationToken = default)
        where T : NamedApiResource
    {
    ...
    public Task<ApiResourceList<T>> GetApiResourcePageAsync<T>(CancellationToken cancellationToken = default)
        where T : ApiResource =>
        InternalGetApiResourcePageAsync<T>(AddPaginationParamsToUrl(GetApiEndpointString<T>()), cancellationToken);
    ...
    public Task<ApiResourceList<T>> GetApiResourcePageAsync<T>(int limit, int offset, CancellationToken cancellationToken = default)
        where T : ApiResource =>
        InternalGetApiResourcePageAsync<T>(AddPaginationParamsToUrl(GetApiEndpointString<T>(), limit, offset), cancellationToken);
    ...
    /// <param name="cancellationToken">Cancellation token for the request; not utilized if data has been cached</param>
    /// <returns>An async enumeration of the requested resources</returns>
    public async IAsyncEnumerable<ApiResource<T>> GetAllApiResourcesAsync<T>([EnumeratorCancellation] CancellationToken cancellationToken = default)
        where T : ApiResource
    {
        string url = GetApiEndpointString<T>();
        bool isLastPage;
